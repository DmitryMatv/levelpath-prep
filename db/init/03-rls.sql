...
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON purchase_orders TO tenant_app;
...
CREATE POLICY tenant_isolation ON purchase_orders USING (
  tenant_id = current_setting('app.tenant_id', true)::uuid
) WITH CHECK (
  tenant_id = current_setting('app.tenant_id', true)::uuid
);
-- missing_ok=true is used here to avoid an error if the setting is not set, which would be a security hole.